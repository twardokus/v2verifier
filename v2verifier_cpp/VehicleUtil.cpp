...
ArgumentParser parse_args  (int argc, char *argv[]){
    ...
    if((std::string)argv[2] == ECDSA || (std::string)argv[2] == FALCON)
        Args.sig_alg = argv[2];
    else{
        perror("Second argument should be either 'ecdsa' or 'falcon'\n");
        exit(EXIT_FAILURE);
    }

    if((std::string)argv[3] == ECDSA || (std::string)argv[3] == FALCON)
        Args.cert_alg = argv[3];
    else{
        perror("Third argument should be either 'ecdsa' or 'falcon'\n");
        exit(EXIT_FAILURE);
    }
...
void help_function(){
    ...
    std::cout<<"\n\nPositional Arguments:\n{receiver,transmitter} - Choice of role\n{ecdsa,falcon} - Signature algorithm to use\n"
               "{ecdsa,falcon} - Certificate algorithm to use\n{implicit,explicit} - Certificate type to use\n{dsrc,cv2x} - Choice of technology"<<std::endl;
    std::cout<<"\n\nOptional Arguments:\n '-h' - Shows this help message and exits\n'--test' - Runs in test mode without SDRs or GNURadio\n\n"<<std::endl;
    std::cout<<"./cpp [-h] {receiver,transmitter} {ecdsa,falcon} {ecdsa,falcon} {implicit,explicit} {dsrc,cv2x} [--test]"<<std::endl;
}
