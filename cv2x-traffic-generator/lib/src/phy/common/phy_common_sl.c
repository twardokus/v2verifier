...
void srslte_cell_sl_fprint(FILE* stream, srslte_cell_sl_t* cell, uint32_t sfn)
{
  fprintf(stream, " - Transmission Mode:            %d\n", cell->tm);
  fprintf(stream, " - SL Id:             %d\n", cell->N_sl_id);
...
bool srslte_cell_sl_isvalid(srslte_cell_sl_t* cell)
{
  if (cell->nof_prb == 50 || cell->nof_prb == 100 ) {
    return true;
  } else {
    return false;
  }
...
/**
 * Set resource pool configurations based on 3GPP TS 36.101 Version 15.6 Sections A.7 and A.9
 *
 * @param q srslte_sl_comm_resource_pool_t object
 * @param cell srslte_cell_sl_t object
 * @param num_sub_channel number of subchannels in cell
 * @param start_prb_sub_channel start prb of subchannel
 * @param adjacency_pscch_pssch subchannelization adjacency
 * @return SRSLTE_SUCCESS if no error occurs, SRSLTE_ERROR if an error occurs
 */
int srslte_sl_comm_resource_pool_set_cfg(srslte_sl_comm_resource_pool_t* q,
                                         srslte_cell_sl_t cell,
                                         uint32_t num_sub_channel,
                                         uint32_t start_prb_sub_channel,
                                         bool adjacency_pscch_pssch)
{
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  if (q != NULL) {
    q->period_length = 40;
    ...
    // TODO: finish resource pool config
    // Use full Bandwidth
    q->prb_num   = (uint32_t)ceil(cell.nof_prb / 2.0);
    q->prb_start = 0;
    q->prb_end   = cell.nof_prb - 1;
    ...
    if (cell.nof_prb % num_sub_channel != 0) {
      ERROR("Invalid nof_prb/num_sub_channel combination. Must be divisible without remainder.");
      return SRSLTE_ERROR;
    }

    q->num_sub_channel       = num_sub_channel;
    q->size_sub_channel      = cell.nof_prb / num_sub_channel;
    q->start_prb_sub_channel = start_prb_sub_channel;
    q->adjacency_pscch_pssch = adjacency_pscch_pssch;
