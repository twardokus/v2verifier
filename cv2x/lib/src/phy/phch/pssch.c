...
uint32_t srslte_pssch_info(srslte_pssch_cfg_t* cfg, char* str, uint32_t str_len)
{
  uint32_t len = srslte_print_check(str, str_len, 0, "prb_start_idx=%d", cfg->prb_start_idx);
  len = srslte_print_check(str, str_len, len, ", nof_prb=%d", cfg->nof_prb);
  len = srslte_print_check(str, str_len, len, ", N_x_id=%d", cfg->N_x_id);
  len = srslte_print_check(str, str_len, len, ", mcs_idx=%d", cfg->mcs_idx);
  len = srslte_print_check(str, str_len, len, ", rv_idx=%d", cfg->rv_idx);
  len = srslte_print_check(str, str_len, len, ", sf_idx=%d \n", cfg->sf_idx);
