...
int srslte_ue_sync_run_track_gnss_mode(srslte_ue_sync_t* q, cf_t* input_buffer[SRSLTE_MAX_CHANNELS])
{
  ...
  uint32_t  offset_samp = round(rx_frac_ms / (1.0 / (q->sf_len * 1000)));

  // if there was a sample offset it should already be corrected, so no new sample offset should be set
  if (q->next_rf_sample_offset) {
    offset_samp = 0;
    q->next_rf_sample_offset = 0;
  }
