...
public:
  ...
  typedef struct {
    mac_tb_t     tb[SRSLTE_MAX_NUM_SUB_CHANNEL];
    srslte_sci_t sci[SRSLTE_MAX_NUM_SUB_CHANNEL];
    uint32_t     rssi[SRSLTE_MAX_NUM_SUB_CHANNEL];
    uint32_t     tti;
  } mac_grant_sl_rx_t;
  ...
  /* Indicate reception of SL.
 * payload_ptr points to memory where MAC PDU must be written by MAC layer */
  virtual void new_grant_sl_tx(mac_grant_sl_tx_t grant, tb_action_sl_t* action) = 0;

  /* Indicate reception of SL. */
  virtual void new_grant_sl_rx(mac_grant_sl_rx_t grant, tb_action_sl_t action[SRSLTE_MAX_NUM_SUB_CHANNEL]) = 0;

  /* Indicate successful decoding of PSSCH AND PCH TB. */
  virtual void tb_decoded(uint32_t subch_idx, mac_grant_sl_rx_t grant) = 0;
...
typedef struct {
  ...
  srslte::channel::args_t sl_rx_channel_args;
  srslte::channel::args_t sl_tx_channel_args;
} phy_args_t;
...
public:
  ...
  virtual void prach_send(uint32_t preamble_idx, int allowed_subframe, float target_power_dbm, float ta_base_sec = 0.0f) = 0;
