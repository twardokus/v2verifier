...
#include "../include/SignedData.hpp"
#include "../include/IEEE1609Dot2DataTypes.hpp"
...
int main() {
    ...
    // Create SignedData
    SignedData s(testCOER);

    if(s.getCOER() != testCOER)
        return 1;

    if(s.getHashID() != IEEE1609Dot2DataTypes::HashAlgorithm::sha256)
        return 2;

    if(s.getTbsData().getCOER() != toBeSignedDataBytes)
        return 3;

    if(s.getSigner().getSignerIdentifierChoice() != SignerIdentifierChoice::self)
        return 4;

    if(s.getSignature().getCOER() != ecdsaP256Bytes)
        return 5;
